  ...
  module Scopes
    ...
    class ScopeBuilder
      ...
      def create_scope
        ...
        elsif name =~ /^(.*)_before$/ && (col = column("#{$1}_at") || column("#{$1}_date")) && col.type.in?([:date, :datetime, :time, :timestamp])
        ...
        elsif name =~ /^(.*)_after$/ && (col = column("#{$1}_at") || column("#{$1}_date")) && col.type.in?([:date, :datetime, :time, :timestamp])
        ...
        elsif name =~ /^(.*)_between$/ && (col = column("#{$1}_at") || column("#{$1}_date")) && col.type.in?([:date, :datetime, :time, :timestamp])
