module Hobo::Rapid
  ...
  def_tag :possessive do
    if current_user == this
      "My"
    elsif display_name.ends_with?('s')
      "#{display_name}'"
    else
      "#{display_name}'s"
    end
  ...
  def_tag :in_place_edit do
    raise HoboError.new("Not allowed to edit") unless can_edit_this?

    if this_type.respond_to?(:macro)
      if this_type.macro == :belongs_to
        in_place_edit_for_belongs_to
      else
        # In place edit for has_many not implemented
        object_link
      end
    elsif this_type.is_in? [:integer, :float, :decimal, :datetime,
                            :date, :timestamp, :time, :text, :string]
      disp = show || "(click to edit)"
  ...
  def_tag :delete_button, :label, :message, :update do
    if can_delete?(this)
      lab = label || "Remove"
  ...
  def_tag :create_button, :model, :update, :attrs, :label, :message do
    ...
    params = attrs || {}
    if model
      model2 = model.is_a?(String) ? model.constantize : model
    else
      ...
      model2 = refl.klass
      params[refl.primary_key_name] = this.proxy_owner.id
    end
    if can_create?(model2.new)
      label2 = label || "New #{model2.name.titleize}"
      message2 = message || label2
      func = ajax_updater(object_url(model2), message2, update,
                          ({:params => { model2.name.underscore => params }} unless params.empty?))
  ...
  AJAX_ATTRS = [:before, :success, :failure, :complete, :type, :method, :script, :form, :params]
